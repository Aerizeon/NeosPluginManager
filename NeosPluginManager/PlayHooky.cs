...
namespace PlayHooky
{
	...
	public class HookManager
	{
        ...
        readonly Dictionary<MethodInfo, byte[]> hooks;
		...
		public HookManager()
		{
            ...
            Assembly.GetExecutingAssembly().ManifestModule.GetPEKind(out _, out ImageFileMachine machine);
		...
		public unsafe void Hook(MethodInfo original, MethodInfo replacement)
		{
			...
			if (original is null) throw new ArgumentNullException("original"); //original function should not be null
			if (replacement is null) throw new ArgumentNullException("replacement"); //replacement function should not be null
		...
		private uint VirtualProtect(IntPtr address, uint size, uint protectionFlags)
		{
            ...
            if (!Natives.VirtualProtect(address, (UIntPtr)size, protectionFlags, out uint oldProtection))
            {
